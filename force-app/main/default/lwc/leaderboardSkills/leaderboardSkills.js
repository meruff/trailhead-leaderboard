...
export default class LeaderboardSkills extends LightningElement {
  ...
  renderedCallback() {
    ...
    loadScript(
      this,
      LEADERBOARD_SOURCE + "/trailheadLeaderboard/assets/Chart.bundle.min.js"
    )
      .then(() => {
        ...
        getSkillData({ userId: this.trailblazerHandle })
          .then((result) => {
            if (result) {
              this.skills = result.sort(
                (a, b) => a.earnedPointsSum < b.earnedPointsSum
              );

              let j = 22;
              for (let i = 0; i < this.skills.length; i++) {
                this.labels.push(this.skills[i].skill.name);
                this.data.push(this.skills[i].earnedPointsSum);
                    ...
                    callbacks: {
                      label: function (tooltipItem, data) {
                        ...
                        let tooltipPercentage =
                          Math.round((tooltipData / total) * 100 * 2) / 2;
                        return (
